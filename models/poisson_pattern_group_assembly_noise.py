...
class PoissonPatternGroup(SpikingGroup):
    ...
    def __init__(self, size, rate=1.0, rate_bg=1.0, params={}):
        ...
        if self.evolve_locally:
            ...
            self.neuron_activation_probability = params.get("neuron_activation_probability", 1.0)
            self.num_simultaneous_assemblies = params.get("num_simultaneous_assemblies", 1)
            ###############
            self.num_simultaneous_noise_assemblies =params.get("num_simultaneous_noise_assemblies", 1)
            ################
    ...
    def evolve(self):
        ...
        if self.pattern_start <= clock < self.pattern_end:
            for assembly, idx_inactive_neurons,noise_assembly in zip(
                    self.active_assemblies, self.idc_inactive_neurons,self.noise_assemblies):
#                 print("active",assembly,"inactive",idx_inactive_neurons,"noise",noise_assembly)
...
#                 print("active",spikes)
                ...
                for spike in spikes:
                    self.push_spike(spike)
                #################################
                mask = self.rng.uniform(size=self.assembly_size) < self.lambd_bg * simulation_timestep
                spikes_bg = self.assembly_neurons_idc[noise_assembly][mask]
#                 print("rank",self.rank_size)
#                 spikes_bg = np.flatnonzero(self.rng.uniform(size=self.rank_size) < self.lambd_bg * simulation_timestep)
#                 print("noise assembly",spikes_bg)
...
#         print("active_assemblies",self.active_assemblies)
############################################################################
    def choose_noise_assemblies(self):
        """TODO"""
#         self.noise_assemblies = self.rng.choice(
#             np.setdiff1d(self.assemblies,self.active_assemblies), self.num_simultaneous_noise_assemblies, replace=False)
        selected_noise = self.rng.choice(
            np.setdiff1d(np.arange(0,8,1),self.active_assemblies), self.num_simultaneous_noise_assemblies, replace=False)
        self.noise_assemblies = [selected_noise[0] for act_assemb in self.active_assemblies]
#         print("noise_assemblies",self.noise_assemblies)
############################################################################
